...
resource "null_resource" "install_mitogen" {
  provisioner "local-exec" {
    command = "pip install ansible-base==2.10 ; ansible --version ; ansible-galaxy install robertdebock.mitogen ; ansible-playbook -i localhost playbooks/install-mitogen.yml"
  }
}
